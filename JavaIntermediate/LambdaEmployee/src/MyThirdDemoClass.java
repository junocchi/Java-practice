...
public class MyThirdDemoClass {
	...
	public static void main(String[] args) {
		List<Employee> empList = Arrays.asList(new Employee(101, "AAAA", "Sales", "Executive", 78000),
				new Employee(102, "BBBB", "IT", "Associate", 67000), 
				new Employee(103, "CCCC", "Sales", "Associate", 0),
				new Employee(104, "DDDD", "IT", "Manager", 0), 
				new Employee(105, "EEEE", "HR", "Executive", 0));
...
//		.forEach(deptt -> System.out.println(deptt));
		...
		//to print the number of "Managers" working in the "Sales" department
		System.out.println("=====================");
		long total=empList.stream()
		.filter(employee -> employee.getDesignation().equals("Manager") && employee.getDepartment().equals("Sales"))
		.count();
		System.out.println("Total No of managers working in sales department : "+total);
		
		
		//to print the names of all employees starting with B
		System.out.println("=====================");
		empList.stream()
		.filter(employee -> employee.getEmpName().charAt(0)=='B').
		forEach(employee -> System.out.println(employee.getEmpName()));
	}
